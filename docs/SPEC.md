...
**detect.py** — 두 규칙을 병렬로 돌린다.

*급증/급감 (z-score)*
- EWMA(span=60)로 기준선 계산
- 잔차의 이동 표준편차로 z-score
- `|z| > 3` 이면 이상 플래그
- 기준선이 `min_baseline` 미만인 구간은 판정하지 않는다 (조용한 구간에서 z가 폭주한다)

*무응답 (outage)*
- 연속 0건이 `outage_minutes` 이상이고, **0이 시작되기 직전 기준선**이
  `min_baseline` 이상이면 무응답
- **가드가 현재 기준선이 아니라 직전 기준선을 본다.** EWMA는 정지를 새 평시로 학습하므로
  현재 기준선은 정지가 길어질수록 0으로 내려간다. 현재 기준선을 쓰면 정지 25분 만에
  감시가 꺼진다

**z-score만으로는 완전 정지를 표현할 수 없다.** 잔차와 척도가 같이 줄어 z가 -2.5 근처에서
멈춘다. z는 "평소보다 적다"를 재는 도구지 "아무것도 안 온다"를 재는 도구가 아니다.
NASA 실측은 `experiments.md` 2026-08-28 / 08-29 항목.

**검증**: 1995-08-01 14:52 ~ 08-03 04:36 구간(허리케인으로 서버 정지)이 잡히는지 확인.
라벨 없이도 정답을 아는 구간이므로 테스트용으로 쓴다.

```bash
uv run python -m src.traffic.detect \
    --input data/interim/nasa_per_min.parquet \
    --output runs/traffic/anomalies.csv \
    --expect-outage "1995-08-01 18:52" "1995-08-03 08:36"
```

**시각 기준 주의.** 위 `14:52 ~ 04:36` 은 NASA README의 **현지시각(EDT, UTC-4)** 이고,
`nasa.csv` 의 `time` 은 UTC epoch 이라 UTC로는 `18:52 ~ 08:36` 이다. 데이터상 실제 정지
시작은 **18:40 UTC (14:40 EDT)** 로 README 기재보다 12분 빠르다. 복구는 정확히 일치한다.
...
| 무응답 `outage_minutes` 분 연속 | `critical` (서비스 정지 의심) |
...
- NASA 로그의 허리케인 정지 구간(1995-08-01 14:52 ~ 08-03 04:36 **EDT**)이 `critical` 로
  뜨는지 — **급감(z)이 아니라 무응답(outage)으로 잡힌다.** 같은 구간의 최저 z는 -1.44 로
  임계 근처에도 못 간다 (`experiments.md` 2026-08-29)
